...
func TestTuistProvider_Detect(t *testing.T) {
	t.Run("detected when binary and Tuist.swift exist", func(t *testing.T) {
		req := mode.DetectRequest{
			Exec: &mode.ExecutorMock{
				LookPathFunc: func(file string) (string, error) {
					return "/usr/local/bin/tuist", nil
				},
				StatFunc: func(name string) (os.FileInfo, error) {
					if name == "Tuist.swift" {
						return nil, nil
		...
		p := mode.TuistProvider{}
		detected, err := p.Detect(t.Context(), req)
		require.NoError(t, err)
		require.True(t, detected)
	})

	t.Run("detected when binary and Tuist directory exist", func(t *testing.T) {
		req := mode.DetectRequest{
			Exec: &mode.ExecutorMock{
				LookPathFunc: func(file string) (string, error) {
					return "/usr/local/bin/tuist", nil
	...
	t.Run("not detected when no markers exist", func(t *testing.T) {
		req := mode.DetectRequest{
			Exec: &mode.ExecutorMock{
				LookPathFunc: func(file string) (string, error) {
					return "/usr/local/bin/tuist", nil
...
func TestTuistProvider_Plan(t *testing.T) {
	t.Run("uses TUIST_XDG_CACHE_HOME with tuist suffix", func(t *testing.T) {
		t.Setenv("TUIST_XDG_CACHE_HOME", "/custom/cache")
		...
		result, err := p.Plan(t.Context(), req)
		require.NoError(t, err)
		require.Equal(t, []string{filepath.Join("/custom/cache", "tuist")}, result.MountPaths)
	})

	t.Run("falls back to XDG_CACHE_HOME with tuist suffix", func(t *testing.T) {
		t.Setenv("TUIST_XDG_CACHE_HOME", "")
		...
		result, err := p.Plan(t.Context(), req)
		require.NoError(t, err)
		require.Equal(t, []string{filepath.Join("/xdg/cache", "tuist")}, result.MountPaths)
	})

	t.Run("TUIST_XDG_CACHE_HOME takes precedence over XDG_CACHE_HOME", func(t *testing.T) {
		t.Setenv("TUIST_XDG_CACHE_HOME", "/tuist/cache")
		t.Setenv("XDG_CACHE_HOME", "/xdg/cache")
		...
		result, err := p.Plan(t.Context(), req)
		require.NoError(t, err)
		require.Equal(t, []string{filepath.Join("/tuist/cache", "tuist")}, result.MountPaths)
	})
	...
	t.Run("uses default path when no env vars set", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, result.MountPaths, 1)
		require.Contains(t, result.MountPaths[0], filepath.Join(".cache", "tuist"))
	})
}
